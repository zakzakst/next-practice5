import { NotificationBannerType } from "../NotificationBanner";
...
const WarningIcon = ({ ...rest }: IconProps) => {
  return (
    <svg
      aria-label="警告"
      fill="none"
...
const ErrorIcon = ({ ...rest }: IconProps) => {
  return (
    <svg
      aria-label="エラー"
      fill="none"
...
const SuccessIcon = ({ ...rest }: IconProps) => {
  return (
    <svg
      aria-label="成功"
      fill="none"
...
type Props = React.ComponentProps<"svg"> & OwnProps;

export const NotificationBannerIcon = ({ type, ...rest }: Props) => {
  switch (type) {
    case "info1":
    case "info2":
